...
function wowClick() {
    ...
    if (mspLoginStat == 1) {
        ...
        if (wowItem.hasClass("popup-target"))
            openPopup(wowItem.data("href"));
    } else {
...
$("#emailLogin").click(function() {
    ...
    $.ajax({
        ...
        if (msg == 'error') {
            ...
            if (responseInfo[4] === "editor") {
                setCookie("msp_user_type", "editor", 365);
                editorPopupInit();
            }
...
function editorPopupInit() {
    if (getCookie("msp_user_type") === "editor") {
        $(".grid-item, .gridItem").filter(function () {
          return $(".editors-pick-wrapper", this).length === 0;
        }).find(".save-button").each(function () {
            var isbn = $(this).closest(".grid-item, .gridItem").data("gridid");
            $(this).data("href", "/promotions/editorspick.php?isbn=" + isbn).addClass("popup-target");
        });
    }
}