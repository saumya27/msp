...
var PriceTable = {
    ...
    "init" : function() {
        ...
        $doc.on("click", ".avlbl-clrs__inpt", (function() {
            ...
            return function() {
                var $this = $(this),
                    ...
                    colorValue = $this.closest('label').data('tooltip');
        ...
        $doc.on("click", ".js-open_msg-box", function handler(e) {
            var $popup = $(".prc-tbl-clm--gts-cshbck-pop");
            if (!$popup.length){
                // ajax call and append the html
                    ...
                    if (json.pricetable) {
                        ...
                        if(_index == -1) {
                            ...
                            if ($(".prc-tbl-row").length > PriceTable.dataPoints.defaultRows) {
                                ...
                                var $priorityStores = $(".prc-tbl-row[data-storename='amazon'], .prc-tbl-row[data-storename='flipkart']").not(":visible");
                                if ($priorityStores.length) {
                                    $priorityStores.insertAfter($(".prc-tbl-row").eq(PriceTable.dataPoints.defaultRows - 1)).show();
                                    PriceTable.dataPoints.defaultRows = $(".prc-tbl-row:visible").length;
                                }
                            } else {
                                $showMoreStores.hide();
                            }
...
$(document).ready(function() {
   ...
   $doc.on("click", ".prc-tbl__no-stck-sbmt", function() {
        var $inputField = $(this).prev(".prc-tbl__no-stck-inpt"),
            $form = $(this).closest(".prc-tbl__no-stck-form"),
        ...
        MSP.utils.validate.form([{
            ...
            $form.hide();
            $form.siblings(".prc-tbl__no-stck-scs").fadeIn();
        });
        return false;
    });
