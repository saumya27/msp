...
function check_fb_login() {
  ...
  FB.getLoginStatus(function(response) {
    if (response.status === 'connected') {
      ...
      $(".userinfo img:first-child")
        .attr('src', 'http://graph.facebook.com/' + response.authResponse
          .userID + '/picture');
...
function update_f_data_login(info) {
  ...
  $.ajax({
      "url": 'https://graph.facebook.com/me/?access_token=' + info.authResponse
        .accessToken,
      "dataType": "json"
    })
    .done(function(data) {
        ...
        $.ajax({
          ...
          loginme_by_fb(email, 'http://graph.facebook.com/' +
            info.authResponse.userID + '/picture');
              $.get(
                'http://www.mysmartprice.com/users/set_username_cookie.php', {
                  email: email
                }, function(name) {
