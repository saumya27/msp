...
(function($, window, document, undefined) {
    ...
    mCycle.prototype = {
        ...
        slideTo: function(index) {
            function multipleSlide(count, direction) {
                if (count) {
                    count--;
                    that.slide(direction);
                    setTimeout(function() {
                        multipleSlide(count, direction);
            ...
            var that = this;
            this.pause();
            var count = index - $(that.element).parent().find(".active").index();
            var direction = count > 0 ? 'left' : 'right';
            multipleSlide(Math.abs(count), direction);
        },
...
$(document).ready(function() {
    $(".js-crsl-wdgt").each(function() {
        ...
        $this.on("click", ".crsl-wdgt__btn-prev", function() {
            $this.mCycle("pause").mCycle("slideRight");
            resetSlideTimeout();
        });
        $this.on("click", ".mCycleSlideBullet", function() {
            $this.mCycle("pause");
            $this.data("mCycle").slideTo($(this).index());
        });
...
$doc.on('click', '.js-category-button, .js-menu-overlay', function(e) {

    $('.js-menu-wrapper').toggleClass('show');
    $('.js-menu-overlay').toggleClass('show');
    if ($('.js-menu-wrapper.show').length !== 0) {
        ...
        if (localStorage && location.hash !== '#forcepopup') {
            ...
            if (diffTime < 30 && localStorage.browsePopupDataVer == $('.js-menu-wrapper')
                .data('ver')) {
        ...
        if (!data || data == 'undefined' || data === undefined) {
            $('.js-menu-wrapper').find('.loading-circle').show();
            data = getBrowsePopupData();
            localStorage.browsePopupData = data;
            localStorage.browsePopupDataTime = new Date().getTime();
            localStorage.browsePopupDataVer = $('.js-menu-wrapper').data('ver');
            // if data is not avaialble in localStorage do ajax and save in localStorage for later use
        }
        if (data && data != 'undefined' && data !== undefined) {
            $('.js-menu-data').html(data).data('processed', 'done');
            setTimeout((function() {
                $('.js-menu-wrapper').find('.loading-circle').hide();
                $('.js-menu-data').addClass('show');
            }), 340);
            // on data available hide loading and show data
        }
    } else {
        $('.js-menu-wrapper').removeClass('show');
        $('.js-menu-data').removeClass('show');
    }
});

$doc.on('click', '.js-menu-wrapper', function(e) {
    e.stopPropagation();
});
// browse menu processing end here

$doc.on('click', '.srch-sbmt', function() {
...
$doc.keyup(function(e) {
    if (e.keyCode == 27) { //esc button
        if ($('.js-menu-overlay.show').length != 0) {
            $('.js-category-button')
