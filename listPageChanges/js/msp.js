...
function initBottomSlideup() {
    $("#promotions").load("/promotions/bottom_banner_promotions.htm", function() {
    ...
    $.fn.sidebarList = function(options) {
        return this.each(function() {
            ...
            if (!$.data(this, "sidebarList")) {
                ...
                $.data(this, "mCycle", new sidebarList(this, options));
            } else {
...
function tryInstallExtension(category, action, callback) {
    if (chrome && chrome.webstore) {
        chrome.webstore.install(CHROME_EXT_INSTALL_URL, function() {
            extensionInstallSuccess(category, action, callback);
        }, function() {
            extensionInstallFail(category, action, callback);
        });
    } else
        extensionInstallFail(category, action, callback);
}
