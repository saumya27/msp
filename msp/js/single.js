	...
	function rating_init(){
		...
		if(rating_anim){
			...
			$('.rating_block').stop().animate({width: '10px'},anim_delay,'linear',function(){rating_anim=false;rating_init();});
		}else{
			var intn = Math.floor(rating_block_val_ori) ;
			var decn =  rating_block_val_ori*10 - intn*10 ;
			if(decn==10) {decn = 0 ; intn++; }
	...
	function rating_count_loop(){
			...
			function filterPriceTable($this){
				...
				$priceTableContainer.css({height:$priceTableContainer.height(),display:'block'});
				...
				if(selectedFilterLength === 0){
					...
					$('.store_pricetable').each(function(i,el){
						...
						$priceTableContainer.css({height:iTo+$El.outerHeight()+'px',display:'block'});
						$El.css({position:'absolute',top:iFr}).stop().animate({top:iTo},500, function(){
							$store_pricetable.css({position:'static',top:'auto'});
							$priceTableContainer.css({height:'auto',display:'block'});
						});
					});
...
function sortPriceTable(){
	...
	if($sortby == 'relevance'){
		...
		$('.store_pricetable').tsort({attr:"data-relrank"}).each(function(i,el){
			var $El = $(el);
			var iFr = $.data(el,'h');
			var iTo = 0;
			$El.prevAll('.store_pricetable:visible').each(function(){
		...
		$('.store_pricetable').tsort({attr:"data-pricerank"}).each(function(i,el){
			var $El = $(el);
			var iFr = $.data(el,'h');
			var iTo = 0;
			$El.prevAll('.store_pricetable:visible').each(function(){
		...
		$('.store_pricetable').tsort({attr:"data-pricerank",order:'desc'}).each(function(i,el){
			var $El = $(el);
			var iFr = $.data(el,'h');
			var iTo = 0;
			$El.prevAll('.store_pricetable:visible').each(function(){
		...
		$('.store_pricetable').tsort('.store_rating_bar_out',{attr:"data-rating",order:'desc'}).each(function(i,el){
			var $El = $(el);
			var iFr = $.data(el,'h');
			var iTo = 0;
			$El.prevAll('.store_pricetable:visible').each(function(){
  ...
  if (hiddenText.length > 0) {
	...
	$('body').on('click',"#addtolistbutton",function(e){
		...
		if(msp_login==1) {
			...
			$.get("/users/add_to_list.php", { mspid:pagemspid } ,function(data) {
			});
		}
		else {
	...
	$('body').on('click','.product_topsec .view_review', function(){
		$('.reviews').click();
		$reviews_out = $('.reviews_out');
		var reviews_outTopPos = $('.reviews_out').offset().top-100;
		$('html, body').stop().animate({scrollTop:reviews_outTopPos});
	});
	$('body').on('click','.tab', function(){
		if($(this).hasClass('selected')) return false;
		if($(this).hasClass('pricecom')){
			if($pricetabbox.length < 1) return true;
			else return false;
		}
		var $pricetabbox = $('.price_table_out');
		if($pricetabbox.length == 1) return true;
		var $tab = $(this);
		var $tabbox = [];
		var classList =$tab.attr('class').split(/\s+/);
		$.each( classList, function(index, item){
			var $tmp_tabbox = $('.'+item+'_out').filter('.tabbox');
			if ($tmp_tabbox.length > 0) {
				$tabbox = $tmp_tabbox;
				location.hash = item;
				return;
			}
	...
	/*selecting other tabs*/
	;(function(){
		if($('#all_review_div').length > 0) {
			return;
		}
		if($('.price_table_out').length < 1){
			var $tabbox = [], $tab = [], hash = (location.hash+'').replace("#","");
			if(hash !== ''){
				$tabbox = $('.'+hash+'_out').filter('.tabbox');
				$tab = $('.'+hash).filter('.tab');
				}
				if($tab.length && $tab.hasClass('pricegraph')){
				if(prgraph === 0)
				create_graph();
				}

			if($tabbox.length === 0 || $tab.length === 0){
				$tab = $('.tab_bar .tab:nth-child(2)');
				var classList =$tab.attr('class').split(/\s+/);
				$.each( classList, function(index, item){
					$tabbox = $('.'+item+'_out').filter('.tabbox');
					if ($tabbox.length > 0) {
						return;
					}
				});
...
function jsAppend(args)
{
   $.ajax({
	...
	cache: false,
		...
		var reviewdiv = document.getElementById("newalertDiv");
...
function formValidator() {
	...
	var email_element=document.getElementById('pricealert-email');
	var email_value=encodeURIComponent(email_element.value);

	var subscribed_status=0;
	var email_subscribed=document.getElementById("pricealert-subscribe").checked;
	if (email_subscribed) subscribed_status=1;

	var mspid = document.getElementById("mspid").value;
	var bestprice = document.getElementById("bestprice").value;

	var text_to_append='?mspid='+mspid+'&email='+email_value+'&bestprice='+bestprice+'&subscribed_status='+subscribed_status;
	jsAppend(text_to_append);
