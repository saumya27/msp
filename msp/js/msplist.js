...
function loadFilterBox() {
    ...
    if (endInr) {
        updateSliderWithDisp(parseInt(startInr, 10), parseInt(endInr, 10));
    }
}
...
function updateSliderWithDisp(a, b) {
    var c = priceVal2SliderVal(a),
        maxSlider = priceVal2SliderVal(b);
    $('#priceSlider').slider('values', [c, maxSlider]);
    updatePriceDisp(a, b);
    requestPriceData(a, b);
    slider2List(a, b);
}
...
function requestPriceData(a, b) {
    if (a == gPriceMin && b == gPriceMax) {
        ...
        addTag(a + ';' + b, a.toLocaleString() + '-' + b.toLocaleString(), 'price');
