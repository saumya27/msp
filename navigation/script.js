...
window.onpopstate = function (event) {
    ...
    if (basePageURL !== window.location.href) { // if it not the page started with
        if (!isSingle(window.location.href)) {
            location.href = location.href; // its a list page and not the page started with so need a reload
            return;
        } else {
...
function resizeHandler() {
    ...
    numGridsPerRow--;
    var finalWidth = (numGridsPerRow * 252);
    $('.list-header-wrapper').width(finalWidth - 16);
    $('.the-grid-wrapper').width(finalWidth);

    if ($(".matching-cats-search").length) $(".matching-cats-search").width(finalWidth - 16);

    if ($('.selected-filters-wrapper .selectedFilters').text() !== "") {
        var selectedFiltersWrapperWidth = finalWidth - 25 - $('.product-text').width();
        if ($(".search-product-text").length)
            selectedFiltersWrapperWidth = finalWidth - 25 - $('.search-product-text').width();
...
function openSingleInit() {
    $('body').on('click', '.grid-item.product .image-wrapper,.grid-item.product .info,.grid-item.product .price-info,.grid-item.product .ql-button', function () {
        ...
        if ($(this).hasClass('ql-button')) {
            ga('send', 'event', 'quicklook', prodId);
        }
        openSingle(prodId);
        return false;
    });
...
var shopMenuHidden = false;
var shopMenuHideTimeOut;
$(window).scroll(function () {
    var $windowScrollTopNew = $(this).scrollTop();
    if ($windowScrollTopNew - $windowScrollTop > 0 && shopMenuHidden === false)
        shopMenuHide();
    else if ($windowScrollTopNew - $windowScrollTop < 0 && shopMenuHidden === true)
        shopMenuShow();
    ...
    //testing autoclose filter
    if (filterTopHop >= 0 && $windowScrollTopNew - filterTopHop > 2500 && $('.filterbar.active').length) {
        filterTopHop = -10;
        $('.filter_btn').click();
    }
    //ending autoclose filter
...
if ($('.shop:not(.active)').length > 0) {
    $('.shop').mouseenter(function () {
        clearTimeout(shopMenuHideTimeOut);
        $('.catMenuNew').show();
        $('.shop').addClass('maintainHover');
        $('.menu').addClass('maintainHover');
    });

    $('.shop').mouseleave(function () {
        shopMenuHideTimeOut = setTimeout(((function () {
            $('.catMenuNew')
                .hide();
            $('.shop')
                .removeClass('maintainHover');
            $('.menu')
                .removeClass('maintainHover');
        })), 500);
    });

    $('.catMenuNew').mouseenter(function () {
        clearTimeout(shopMenuHideTimeOut);
        $('.catMenuNew').show();
        $('.shop').addClass('maintainHover');
        $('.menu').addClass('maintainHover');
    ...
    $('.catMenuNew').mouseleave(function () {
        shopMenuHideTimeOut = setTimeout(((function () {
            $('.catMenuNew')
                .hide();
            $('.shop')
...
function shopMenuShow() {
    shopMenuHidden = false;
    $('.catMenuNew').stop().animate({
        top: '40px'
    }, 500);
}
...
function shopMenuHide() {
    shopMenuHidden = true;
    $('.catMenuNew').stop().animate({
        top: '-' + ($('.catMenuNew').outerHeight() - 40) + 'px'
    }, 500);
}
...
function filterInit(action) {
    ...
    if (action == 'init') { //only for init not for reinit
        ...
        //show hide filter init
        $('.filter_btn').on('click', function () {
            if (!$('.filterbar').hasClass('active')) return;
            toggleFilterBar('close');
            return false;
        });
        $('.filterbar').on('click', function () {
            if ($('.filterbar').hasClass('active')) return;
            toggleFilterBar('open');
            return false;
        });
        $('.filterbar').on('mouseenter', function () {
            $('.filterbar').addClass('hover');
            return false;
        });
        $('.filterbar').on('mouseleave', function () {
        ...
        $("body").on("click", ".filterTitle", function () {
            ...
            if (filterBody.closest(".filterBlock").hasClass("min")) {
                calcOverflowFilter(filterBody.closest(".filterBlock").attr('data-groupname'));
            } else {
...
function toggleFilterBar(action) {
    if (action == 'open')
    ...
    if (!$('.filterbar').hasClass('active')) {
        $('.filter_btn span.text').text("SHOW FILTERS");
        ga('send', 'event', 'filters', 'hide');
    } else {
        $('.filter_btn span.text').text("HIDE FILTERS");
        filterTopHop = $(window).scrollTop(); //updating filterhop after filter open for autohide filter
        ga('send', 'event', 'filters', 'show');
    }
    return false;
}

function filterButtonBig() {
    $('.filter_btn').removeClass('scrolled');
}

function filterButtonSmall() {
    $('.filter_btn').addClass('scrolled');
}
...
function calcOverflowFilter(grpname) {
    var margin = 0,
        padding = 10,
        contentLength = $('.filterBlock').length,
        contentTitleHeight = $('.filterTitle').outerHeight() * contentLength,
        topOffset = parseInt($('.filterbar').css('top'), 10),
        filterboxUsableHeight = window.innerHeight - topOffset;
    $('.filterBlock').each(function () {
        margin += parseInt($(this).css('marginTop'), 10) + parseInt($(this).css('marginBottom'), 10);
    });
    filterboxUsableHeight -= (contentTitleHeight + margin + padding);
    if (grpname) {
        filterboxUsableHeight -= $('.filterBlock[data-groupname="' + grpname + '"] .filterBody').outerHeight();
        $('.filterBlock[data-groupname="' + grpname + '"]').removeClass("min");
        $('.filterBlock[data-groupname="' + grpname + '"]').find('.filterBody').slideDown(function () {
            $('.filterBlock[data-groupname="' + grpname + '"]').removeClass("collapsed");
        });
    }
    if (lastSelectGroup !== '') {
        filterboxUsableHeight -= $('.filterBlock[data-groupname="' + lastSelectGroup + '"] .filterBody').outerHeight();
        $('.filterBlock[data-groupname="' + lastSelectGroup + '"]').removeClass("min");
        $('.filterBlock[data-groupname="' + lastSelectGroup + '"]').find('.filterBody').slideDown(function () {
            $('.filterBlock[data-groupname="' + lastSelectGroup + '"]').removeClass("collapsed");
        });
    }
    $('.filterBody:visible').each(function () {
        if ($(this).closest('.filterBlock').attr('data-groupname') == grpname) return;
        if ($(this).closest('.filterBlock').attr('data-groupname') == lastSelectGroup) return;
        if ($(this).closest('.filterBlock').hasClass('min')) return;
        filterboxUsableHeight -= $(this).outerHeight();
        if (filterboxUsableHeight < 0) {
            $(this)
                .closest(".filterBlock")
                .addClass("min");
            $(this)
    ...
    for (var member in filterArr) {
        ...
        for (var val in filterVal) {
            ...
            if (filterName == 'price') {
                ...
                filterDispVal = $('input[name="' + filterName + '"][value="' + filterVal[val] + '"]').closest('.filterItem').attr('title');
