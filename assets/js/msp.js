...
$doc.on('click', '.popup-target', function() {
    ...
    if ($this.hasClass("storebutton")) storeUrl = $this.attr('href');
...
$(function() {
    ...
    var recent_data = $('.recentHelper'), recent, item_data;
...
function initCouponSlideup() {
  var hideFlipkartSlideup = getCookie("hideFlipkartSlideup");
  if (hideFlipkartSlideup !== "true") {
    $("body").on("click", ".coupon-slideup .close-button", function () {
      hideSlideup();
      return false;
    });
    $("#promotions").load("/promotions/flipkartfashionsale.html", function () {
...
function hideSlideup() {
  ...
  addCookie("hideFlipkartSlideup", "true", 1);
}
