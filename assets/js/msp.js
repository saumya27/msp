...
function setSearchUrl(e) {
    var selected = $('.search-cat :selected')
        .data('category');
...
// If cookie is not set, download and show the slide-up banner, and hook up the buttons' click event
function initCouponSlideup() {
  var hideCouponSlideup = getCookie("hideCouponSlideup");
  if (hideCouponSlideup !== "true") {
    $("body").on("click", ".coupon-slideup .btn, .coupon-slideup .close-button", hideSlideup);
    $("#promotions").load("/promotions/ccdblogbanner.html", function () {
      setTimeout(showSlideup, 1500);
    });
  }
}
